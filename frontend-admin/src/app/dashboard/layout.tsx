...
import { LayoutDashboard, ShoppingBag, Package, Tag, Settings, LogOut, Menu, X, MessageSquare, Users, Megaphone, MapPin } from "lucide-react";
...
const NAV = [
  ...
  { href: "/dashboard/zonas-entrega", label: "Zonas de Entrega", icon: MapPin, exact: false, roles: ["ADMIN", "GESTOR"] },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (item?.roles && !item.roles.includes(user.role as never)) {
      router.replace("/dashboard");
    }
  }, [user, pathname, isReady, router]);
  ...
  function NavLinks() {
    return (
      <>
        ...
        <div className="px-3 py-4 border-t border-gray-100/60 dark:border-white/5 space-y-1">
          {user && (
